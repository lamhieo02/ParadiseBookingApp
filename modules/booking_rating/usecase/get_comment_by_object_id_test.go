...
func TestGetCommentByObjectID(t *testing.T) {
	// init mock
	...
	Convey("Test Get Comment By Object ID", t, func() {
		Convey("Get Booking Rating by ID fail", func() {
			...
			_, err := uc.GetCommentByObjectID(ctx, bookingID, objectType)
			...
			Convey("Get Profile By ID success", func() {
				...
				Convey("Object Type = Place", func() {
					...
					Convey("Get Place By ID fail", func() {
						mockPlaceSto.EXPECT().GetPlaceByID(ctx, gomock.Any()).Return(&entities.Place{}, errors.New("error"))
						_, err := uc.GetCommentByObjectID(ctx, bookingID, objectType)
						So(err, ShouldNotBeNil)
					})
				...
				Convey("Object Type = Post Guide", func() {
					...
					Convey("Get PostGuide By ID fail", func() {
						mockPostGuideSto.EXPECT().GetByID(ctx, gomock.Any()).Return(&entities.PostGuide{}, errors.New("error"))
						_, err := uc.GetCommentByObjectID(ctx, bookingID, objectType)
						So(err, ShouldNotBeNil)
					})
					Convey("Get PostGuide ID success", func() {
						mockPostGuideSto.EXPECT().GetByID(ctx, gomock.Any()).Return(&entities.PostGuide{}, nil)
						_, err := uc.GetCommentByObjectID(ctx, bookingID, objectType)
						So(err, ShouldBeNil)
					})
				})
